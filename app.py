...
def summarize_and_speech(pdf_file, text1):
    ...
    summary = summarization(abstract_text, max_length=15, min_length=10, promptstr=text1)
    ...
    return summary #, imagetext #, audio_data

def summarization(text, max_length=15, min_length=10, promptstr="Summarize the above provided paper and show highlights?"):

    message_text = [
        {"role" : "system",
        "content" : "You are an AI assistant that helps people find information."},
        {"role" : "user", "content" : promptstr + " " + text}
    ]
    ...
    response = client.chat.completions.create(
        model="gpt-4-turbo", # model = "deployment_name". Ggpt4, gpt4 version 1106
...
iface = gr.Interface(
    ...
    inputs=[gr.File(label="Upload PDF", type="binary"), 
            gr.Textbox(label="Text 1", info="Initial text", lines=3, value="Summarize the above provided paper and show highlights?",), 
            #gr.Image(type="pil"),
            ],
    outputs=[gr.Textbox(label="Abstract Summary:", autoscroll=True, show_copy_button=True)],
    live=True,
